 ...
 * <p>
 * Users will be redirected to Google Play by default, but any other app store can be used by providing a custom URI
 * (via setTargetUri(...))
 * <p>
 * This library uses English default phrases but you may provide custom phrases either as Strings or resource IDs (via
 * setPhrases(...))
 * <p>
 * Users will be prompted to rate your app after at least 2 days and 5 app launches (customizable via
 * setDaysBeforePrompt(...) and setLaunchesBeforePrompt(...))
 * <p>
 * You may overwrite the default preference group name and key names by calling setPreferenceKeys(...) (usually not
 * necessary)
 * <p>
 * The prompting dialog will be displayed (if adequate) as soon as you call show() on your AppRater instance
 * <p>
 * The dialog will only be shown if at least one application is available on the user's phone to handle the Intent that
 * is defined by the target URI
 * <p>
...
public class AppRater {
    ...
    private static final int DEFAULT_LAUNCHES_BEFORE_PROMPT = 0;
     ...
     * 
     * @param context
     *            the Activity reference to use for this instance (usually the Activity you called this from)
     */
     ...
     *            the Activity reference to use for this instance (usually the Activity you called this from)
     * @param packageName
     *            your application's package name that will be used to open the ratings page
     */
     ...
     * 
     * @param days
     *            the number of days to wait before showing the prompt
     */
     ...
     * 
     * @param launches
     *            the number of launches to wait before showing the prompt
     */
     ...
     * 
     * @param uri
     *            the target URI to open when the user wants to rate the app (must include `%s`)
     */
     ...
     * 
     * @param title
     *            the title for the prompt window (as a string)
     * @param explanation
     *            the explanatory text that will be shown inside the prompt window (as a string)
     * @param buttonNow
     *            the caption for the `Rate now` button (as a string)
     * @param buttonLater
     *            the caption for the `Maybe later` button (as a string)
     * @param buttonNever
     *            the caption for the `Never` button (as a string)
     */
    public void setPhrases(final String title, final String explanation, final String buttonNow,
            final String buttonLater, final String buttonNever) {
        mText_title = title;
     ...
     * 
     * @param title
     *            the title for the prompt window (as a resource ID)
     * @param explanation
     *            the explanatory text that will be shown inside the prompt window (as a resource ID)
     * @param buttonNow
     *            the caption for the `Rate now` button (as a resource ID)
     * @param buttonLater
     *            the caption for the `Maybe later` button (as a resource ID)
     * @param buttonNever
     *            the caption for the `Never` button (as a resource ID)
     */
    public void setPhrases(final int title, final int explanation, final int buttonNow, final int buttonLater,
            final int buttonNever) {
        try {
    ...
    public AlertDialog show() {
        ...
        if (launch_count >= mLaunchesBeforePrompt) { // wait at least x app launches
            if (System.currentTimeMillis() >= (firstLaunchTime + (mDaysBeforePrompt * DateUtils.DAY_IN_MILLIS))) { // wait
                                                                                                                   // at
                                                                                                                   // least
                                                                                                                   // x
                                                                                                                   // days
    ...
    private void resetLaunchCount(SharedPreferences.Editor editor) {
        if (editor != null) {
            editor.putLong(mPreference_launchCount, 0);
            savePreferences(editor);
        }
    }
    ...
    void buttonNowClick(SharedPreferences.Editor editor, DialogInterface dialog, Context context) {
        setDontShow(editor);
        closeDialog(dialog);
        context.startActivity(mTargetIntent);
    }

    void buttonLaterClick(SharedPreferences.Editor editor, DialogInterface dialog) {
        resetLaunchCount(editor);
        closeDialog(dialog);
    }

    void buttonNeverClick(SharedPreferences.Editor editor, DialogInterface dialog) {
        setDontShow(editor);
        closeDialog(dialog);
    }
